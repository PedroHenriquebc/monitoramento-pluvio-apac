...
<body>
    ...
    $estacoes_vistas = []; // Para rastrear códigos GMMC já exibidos

    foreach ($filtered_data as $entry) {
        $codigo_gmmc = $entry['codigo_gmmc'];
        if (in_array($codigo_gmmc, $estacoes_vistas)) {
            continue; // Pular se o código GMMC já tiver sido exibido
        }
        ...
        // Adicionar código GMMC à lista de visualizados
        $estacoes_vistas[] = $codigo_gmmc;
